 ...
 * @brief   ERP Mini platform serial bridge
 * @author  Minkyu Kil
 * @date    2025-06-17
 * @version 1.0
 *
...
#include "erp_mini_serial/serial_packet.hpp"
 ...
 */
erp_mini_serial::SerialBridge::SerialBridge(const rclcpp::NodeOptions &options)
 ...
 */
erp_mini_serial::SerialBridge::~SerialBridge()
{
    // Close and deallocate serial port
    if(!serial_port_)
    {
        ERP_MINI_ERROR("SerialBridge::~SerialBridge() Serial port %s is deallocated already.", 
            port_path_.c_str());
    }
    else
    {
 ...
 */
bool erp_mini_serial::SerialBridge::receive_feedback()
{
    ...
    {
        ERP_MINI_ERROR("SerialBridge::receive_feedback() Packet protocol is ruined.");
        return false;
    }

    // Byte to ROS message
    erp_mini_msgs::msg::Feedback feedback_msg;
 ...
 */
bool erp_mini_serial::SerialBridge::transmit_command()
{
 ...
 */
void erp_mini_serial::SerialBridge::timer_callback()
{
    transmit_command();
    receive_feedback();
    heartbeat_++;
 ...
 */
void erp_mini_serial::SerialBridge::mode_command_callback(
    const erp_mini_msgs::srv::ModeCommand::Request::SharedPtr request,
    erp_mini_msgs::srv::ModeCommand::Response::SharedPtr response
)
{
    // Control mode
 ...
 */
void erp_mini_serial::SerialBridge::control_command_callback(const erp_mini_msgs::msg::ControlCommand::SharedPtr msg)
{
...
void erp_mini_serial::SerialBridge::initialize_node()
{
    // Timer
    timer_ = this->create_wall_timer(20ms, std::bind(&SerialBridge::timer_callback, this));

    // Publishers
    feedback_pub_ = this->create_publisher<erp_mini_msgs::msg::Feedback>(
        "/erp_mini/feedback",
        rclcpp::QoS(rclcpp::KeepLast(1)).reliable().durability_volatile()
    );

    // Subscribers
    control_command_sub_ = this->create_subscription<erp_mini_msgs::msg::ControlCommand>(
    ...
    // Services
    mode_command_srv_ = this->create_service<erp_mini_msgs::srv::ModeCommand>(
        "/erp_mini/mode_command",
    ...
    if(!serial_port_)
    {
        throw erp_mini_util::Exception("SerialBridge::initialize_node() Serial port allocation failed.");
    }
    else
    {
...
/** @brief Declares and retrieves ROS2 parameters for serial and ERP Mini configuration. */
void erp_mini_serial::SerialBridge::declare_parameters()
{
    // Serial port
    ...
    this->declare_parameter<double>("max_steering_deg", 17.0);
...
RCLCPP_COMPONENTS_REGISTER_NODE(erp_mini_serial::SerialBridge)