 ...
 * @brief   Manages serial port communication for ERP Mini platform
 * @author  Minkyu Kil
 * @date    2025-06-17
 * @version 1.0
 *
...
#include "erp_mini_serial/serial_port.hpp"
#include "erp_mini_util/exception.hpp"
 ...
 */
erp_mini_serial::SerialPort::SerialPort(const std::string &port_path, const int &baud_rate)
 ...
 */
void erp_mini_serial::SerialPort::open_port()
{
    if(port_path_.empty())
    {
        file_descriptor_ = -1;
        throw erp_mini_util::Exception("SerialPort::open_port() Port path is empty.");
    }
    ...
    {
        file_descriptor_ = -1;
        throw erp_mini_util::Exception("SerialPort::open_port() File descriptor opening error.");
    }

    this->initialize_port();
    ERP_MINI_INFO("SerialPort::open_port() Serial port %s has been opened successfully.",
        port_path_.c_str());
}
 ...
 */
void erp_mini_serial::SerialPort::close_port()
{
    if (file_descriptor_ < 0) 
    {
        file_descriptor_ = -1;
        throw erp_mini_util::Exception("SerialPort::close_port() Invalid file descriptor.");
    }

    if (close(file_descriptor_) != 0) 
    {
        file_descriptor_ = -1;
        throw erp_mini_util::Exception("SerialPort::close_port() Failed to close file descriptor.");
    }

    file_descriptor_ = -1;
    ERP_MINI_INFO("SerialPort::close_port() Serial port %s has been closed successfully.",
        port_path_.c_str());
}
 ...
 */
bool erp_mini_serial::SerialPort::receive_packet(unsigned char *rx_packet, const unsigned int &expected_packet_size) const
{
    if(file_descriptor_ < 0)
    {
        ERP_MINI_ERROR("SerialPort::receive_packet() Port is not opened.");
        return false;
    }
    ...
    {
        ERP_MINI_ERROR("SerialPort::receive_packet() Expected size is %d, but received size is %zd.",
 ...
 */
bool erp_mini_serial::SerialPort::transmit_packet(
    const unsigned char *tx_packet, const unsigned int &expected_packet_size
) const
{
    if(file_descriptor_ < 0)
    {
        ERP_MINI_ERROR("SerialPort::transmit_packet() Port is not opened.");
        return false;
    }
    ...
    {
        ERP_MINI_ERROR("SerialPort::transmit_packet() Expected size is %d, but transmitted size is %zd.",
 ...
 */
void erp_mini_serial::SerialPort::initialize_port()
{
    struct termios tty;
    ...
    {
        file_descriptor_ = -1;
        throw erp_mini_util::Exception("SerialPort::initialize_port() tcgetattr failed.");
    }
    ...
    default:
        file_descriptor_ = -1;
        throw erp_mini_util::Exception("SerialPort::initialize_port() Invalid baud_rate, use 9600 or 115200.");
    }
    ...
    {
        file_descriptor_ = -1;
        throw erp_mini_util::Exception("SerialPort::initialize_port() tcsetattr failed.");
    }
}