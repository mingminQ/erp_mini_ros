...
namespace // anonymous namespace for internal linkage
{
    ...
    static const char *log_header[4] = {"[ERP MINI Debug]: "   ,
                                        "[ERP MINI Info]: "    ,
                                        "[ERP MINI Warning]: " ,
                                        "[ERP MINI Error]: "   };
    ...
    struct LogManager
    {
        ...
        erp_mini_util::LogLevel log_level_;

        // Activated handler type
        erp_mini_util::LogType log_type_;

        // Pointer to active handler
        erp_mini_util::LogHandler *current_handler_;

        // Terminal log handler instance
        erp_mini_util::LogHandlerTerminal *terminal_log_handler_;

        //  File handler instance, if enabled
        erp_mini_util::LogHandlerFile *file_log_handler_;
        ...
        LogManager()
        {
            ...
            log_level_ = erp_mini_util::LogLevel::INFO;

            // File log handler is not activated at default
            terminal_log_handler_ = new erp_mini_util::LogHandlerTerminal();
            file_log_handler_     = nullptr;

            // Default log handler is terminal log handler
            log_type_        = erp_mini_util::LogType::TERMINAL;
 ...
 */
void erp_mini_util::LogHandlerTerminal::print_log(const char *file_name, int line_number, LogLevel log_level, const std::string &msg)
{
    // Linux system only
    // bool isTTY(isatty(fileno(stderr)) != 0);
    bool isTTY = true;

    if(erp_mini_util::LogLevel::WARNING <= log_level)
    {
        if(isTTY)
        {
    ...
    else // log_level <= erp_mini_util::LogLevel::INFO
    {
        if(isTTY)
        {
 ...
 */
erp_mini_util::LogHandlerFile::LogHandlerFile(const char *file_name)
{
...
erp_mini_util::LogHandlerFile::~LogHandlerFile()
{
    if(file_ != nullptr)
    {
        if(fclose(file_) != 0)
 ...
 * @param msg Formatted log message
 */
void erp_mini_util::LogHandlerFile::print_log(const char *file_name, int line_number, LogLevel log_level, const std::string &msg)
{
    if(file_ != nullptr)
    {
        if(erp_mini_util::LogLevel::WARNING <= log_level)
 ...
 */
void erp_mini_util::set_log_level(LogLevel log_level)
{
    GET_LOG_MANAGER;
 ...
 */
erp_mini_util::LogLevel erp_mini_util::get_log_level()
{
    GET_LOG_MANAGER;
 ...
 * @param file_name Path to the log file
 */
void erp_mini_util::activate_log_file(const std::string &file_name)
{
    GET_LOG_MANAGER;
...
void erp_mini_util::deactivate_log_file()
{
    GET_LOG_MANAGER;
    if(log_manager->file_log_handler_ == nullptr)
    {
 ...
 */
void erp_mini_util::set_log_type(LogType log_type)
{
    GET_LOG_MANAGER;
    if(log_type != log_manager->log_type_)
    {
        ...
        else // log_type == erp_mini_util::TERMINAL_HANDLER
        {
 ...
 */
erp_mini_util::LogType erp_mini_util::get_log_type()
{
    GET_LOG_MANAGER;
 ...
 */
void erp_mini_util::print_log(const char *file_name, int line_number, LogLevel log_level, const char *msg, ...)
{
    GET_LOG_MANAGER;
